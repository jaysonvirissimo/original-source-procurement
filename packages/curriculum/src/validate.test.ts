...
/** Every mission is on the default path unless a test says otherwise. */
function data(overrides: Partial<CurriculumData>): CurriculumData {
  const missions = overrides.missions ?? [];
  return {
    skills: [skill("S.A"), skill("S.B", ["S.A"])],
    manualEntries: [ENTRY],
    missions,
    defaultPath: missions.map((value) => (value as { id: string }).id),
    ...overrides,
  };
}
...
describe("validateCurriculum", () => {
  ...
  it("reports a mission the default path leaves out", async () => {
    const issues = await validateCurriculum({
      ...curriculum,
      defaultPath: curriculum.defaultPath.filter((id) => id !== "035"),
    });
    expect(issues.map(({ code, message }) => [code, message])).toContainEqual([
      "missing-from-path",
      "Mission 035 is not on the default path.",
    ]);
  });

  it("reports a target generated by another toolchain", async () => {
    const stale = await mission({ id: "001", teaches: [] });
    if (stale.target.kind !== "inline") {
      throw new Error("expected an inline target");
    }
    stale.target.toolchain.psyqAsmVersion = "0.1.0";
    const issues = await validateCurriculum(
      data({ missions: [stale], defaultPath: ["001"] }),
    );
    expect(
      ...
      [
        "target-toolchain",
        "missions[0].target.toolchain.psyqAsmVersion",
        "psyqAsmVersion is 0.1.0, but the pinned toolchain is 0.2.0. Run pnpm curriculum:targets.",
      ],
    ]);
  });
  ...
  it("reports a mission that fails its schema", async () => {
    expect(
      await codes(
        data({
          missions: [await mission({ kind: "synthesis" })],
          defaultPath: [],
        }),
      ),
    ).toEqual(["schema"]);
  ...
  it("reports an unknown skill in a prerequisite and in a mission", async () => {
    ...
    expect(issues).toEqual([
      ...
      },
      {
        ...
        path: "defaultPath[0]",
        message:
          "Mission sample-training needs S.UNKNOWN, which no earlier mission on the default path teaches.",
      },
    ]);
  });
